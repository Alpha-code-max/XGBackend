const getHomePageHtml = (user) => {
  ...
  if (user) {
    ...
    html += `
      <div class="auth-box">
        <p>Logged in as <strong>${user.displayName || user.username}</strong></p>
        ${user.photo ? `<img src="${user.photo}" alt="Profile" width="100" style="border-radius:50%"><br><br>` : ''}
...
const getProtectedPageHtml = (user) => {
  return `
